...
class AsrScorer:
    ...
    def score_asr_result(self, asr_text: str, context: Optional[str] = None) -> AsrScore:
        """对ASR识别结果进行评分
    ...
    def _rule_based_scoring(self, text: str, context: Optional[str] = None) -> Dict[str, float]:
        """基于规则的评分
        ...
        if sentence_count > 0:
            ...
            logic_ratio = min(logic_count / sentence_count * 10, 1.0)  # 每句最多0.1个逻辑词
            scores["logic_score"] += logic_ratio * 20  # 最多加20分
        
        # 2. 语义通顺度评分
        # 检查常见搭配和词频
        common_words = ["的", "了", "在", "是", "我", "有", "和", "就", "人", "都", "也", "很", "到", "说", "要"]
        word_count = len(text)
        if word_count > 0:
    ...
    def _ai_based_scoring(self, text: str, context: Optional[str] = None) -> Optional[Dict[str, float]]:
        """基于AI的评分（使用大语言模型）"""
        try:
            # 构建评分提示词
            ...
            response = self.client.chat.completions.create(
                model="qwen-max",
                messages=[
                    {"role": "system", "content": "你是一个专业的语音识别质量评估专家。"},
...

{
        ...
        try:
            ...
            if json_match:
                ...
                return {
                    ...
                    "semantic_coherence": float(score_data.get("semantic_coherence", 50.0)),
                    "context_consistency": float(score_data.get("context_consistency", 50.0))
                }
        except Exception as e:
    ...
    def _detect_errors(self, text: str, context: Optional[str] = None) -> Dict[str, Any]:
        """检测ASR识别错误并提供校正建议"""
        corrections = []
        ...
        for error_text, mapping in self.error_mappings.items():
            if error_text in text:
                # 检查上下文是否匹配
                should_correct = self._check_context_match(text, context, mapping.get("context_keywords", []))
    ...
    def _check_context_match(self, text: str, context: Optional[str], keywords: List[str]) -> bool:
        """检查上下文是否匹配关键词"""
        if not keywords:
    ...
    def _combine_scores(self, rule_scores: Dict[str, float], ai_scores: Optional[Dict[str, float]]) -> Dict[str, float]:
        """合并规则评分和AI评分"""
        ...
        for key in rule_scores.keys():
            combined[key] = ai_scores.get(key, rule_scores[key]) * 0.7 + rule_scores[key] * 0.3
    ...
    def _generate_suggestions(self, scores: Dict[str, float], error_detection: Dict[str, Any]) -> List[str]:
        """生成改进建议"""
        suggestions = []
        ...
        for correction in corrections:
            suggestions.append(f"建议将 '{correction['error']}' 校正为 '{correction['corrected']}'")
