...
class TranslationScorer:  # pylint: disable=too-many-instance-attributes
    ...
    def score_translation(
        ...
        weights = self.style_weights.get(
            style_enum, self.style_weights[VideoStyle.AUTO]
        )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        sanitized_response = (
                            LLMOutputValidator.sanitize_translation_output(raw_response)
                        )
    ...
    def _sanitize_score_data(self, score_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 处理suggestions字段
            if "suggestions" in score_data and isinstance(
                score_data["suggestions"], list
            ):
                # 对每个建议进行安全验证
            ...
            if "detailed_feedback" in score_data and isinstance(
                score_data["detailed_feedback"], str
            ):
                try:
                    # 使用自定义的安全验证，不转义HTML
                    safe_feedback = self._safe_text_clean(
                        score_data["detailed_feedback"]
                    )
...
5. 不要在JSON前后添加任何其他文字或标记"""
    ...
    def _parse_score_data(
        self, score_data: Dict[str, Any], weights: Dict[str, float]
    ) -> TranslationScore:
        """解析评分数据"""
        ...
        cultural_adaptation = min(
            100, max(0, int(score_data.get("cultural_adaptation", 60)))
        )
        ...
        print(
            f"[评分] 流畅度: {fluency}, 完整性: {completeness}, 一致性: {consistency}"
        )
        print(
            f"[评分] 准确性: {accuracy}, 风格适配: {style_adaptation}, 文化适配: {cultural_adaptation}"
        )
        print(
            f"[评分] 综合得分: {overall_score:.1f}, 阈值: {SCORE_THRESHOLD}, 是否重试: {should_retry}"
        )
    ...
    def _parse_text_response(
        self, response: str, weights: Dict[str, float]
    ) -> TranslationScore:
        """解析文本格式的响应"""
...
参考译文："""
        ...
        try:
            ...
            try:
                ...
                if content is None:
                    print("[参考译文] LLM返回空响应，返回空字符串")
                    return ""

                reference_text = LLMOutputValidator.sanitize_translation_output(content)
                print("[参考译文] 参考译文生成成功")
