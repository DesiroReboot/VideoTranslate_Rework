...
class AIServices:
    ...
    def __init__(self, translation_style: str = "auto"):
        ...
        translation_style = InputValidator.validate_text_input(
            translation_style, max_length=50, min_length=1, context="翻译风格"
        )
        ...
        self.openai_client = OpenAI(
            ...
            timeout=ResourceValidator.validate_timeout(
                120.0, max_timeout=300.0
            ),  # 120秒超时
        )
    ...
    def _upload_to_oss(file_path: str, expiration=3600) -> str:
        ...
        try:
            ...
            print(
                f"[OSS] 连接配置 - Endpoint: {OSS_ENDPOINT}, Bucket: {OSS_BUCKET_NAME}"
            )
            ...
            try:
                ...
                for i in range(max_retries):
                    ...
                    if task_status == "SUCCEEDED":
                        # 获取识别结果
                        transcription_url = result_response.output["results"][0][
                            "transcription_url"
                        ]
                        print("[ASR] 识别完成, 下载结果...")
                        ...
                        if not text:
                            # 尝试从句子中提取
                            sentences = result_data.get("transcripts", [{}])[0].get(
                                "sentences", []
                            )
                        ...
                        if self.asr_scorer:
                            ...
                            score_result = self.asr_scorer.score_asr_result(
                                original_text
                            )

                            print(
                                f"[ASR] 识别质量评分: {score_result.overall_score}/{100}"
                            )
                            if score_result.corrections:
                                print(
                                    f"[ASR] 发现{len(score_result.corrections)}处可能的识别错误，正在校正..."
                                )
                                text = self.asr_scorer.apply_corrections(
                                    original_text, score_result.corrections
                                )
                                print(
                            ...
                            # 保存评分结果
                            self._save_asr_score_result(
                                original_text, score_result, audio_path=audio_path
                            )

                            # 检查是否需要重试
                            if (
                                score_result.should_retry
                                and retry_count < ASR_MAX_RETRIES
                            ):
                                print(
                                    f"[ASR] 识别质量低于阈值({ASR_SCORE_THRESHOLD})，将进行重试"
                                )
                                continue  # 重试识别
                    ...
                    if task_status == "FAILED":
                        ...
                        print(
                            f"[ASR] 任务状态: {task_status}, 等待中... ({i + 1}/{max_retries})"
                        )
                        time.sleep(2)  # 等待2秒
                    else:
    ...
    def translate_text(
        ...
        try:
            ...
            print(
                f"[翻译] 模型参数: temperature={model_params.get('temperature', 0.3)}, top_p={model_params.get('top_p', 0.8)}"
            )
                ...
                try:
                    ...
                    if "timeout" in str(e).lower() or "timed out" in str(e).lower():
                        if attempt < max_retries - 1:
                            print(
                                f"[翻译] 请求超时，{retry_delay}秒后重试 ({attempt + 1}/{max_retries})"
                            )
            ...
            try:
                ...
                translated_text = LLMOutputValidator.sanitize_translation_output(
                    completion.choices[
                        0
                    ].message.content  # VALIDATED: Immediately sanitized
                )
                print("[翻译] 安全验证通过")
            except OutputValidationError as e:
                print("[翻译] 安全验证失败: {e}")
            ...
            print(
                f"[翻译] 译文: {translated_text[:100]}..."
                if len(translated_text) > 100
                else f"[翻译] 译文: {translated_text}"
            )
            ...
            if corrected_text != translated_text:
                ...
                print(
                    f"[翻译] 修正后: {corrected_text[:100]}..."
                    if len(corrected_text) > 100
                    else f"[翻译] 修正后: {corrected_text}"
                )
            else:
                print("[翻译] 无需词典修正")
    ...
    def _translate_with_adjusted_params(
        ...
        try:
            # 调用Qwen-max API
            completion = self.openai_client.chat.completions.create(
                model=MT_MODEL, messages=messages, **adjusted_params
            )
    ...
    def _save_asr_score_result(
        ...
        try:
            ...
            score_data = {
                ...
                "context": context,
                "asr_text": asr_text,
                "corrected_text": self.asr_scorer.apply_corrections(
                    asr_text, score.corrections
                )
                if self.asr_scorer
                else asr_text,
                "scores": {
    ...
    def text_to_speech(
        ...
        try:
            ...
            error_message = str(e)
            print(f"[TTS] 原始错误: {error_message}")
            # 检查错误是否与字符长度相关
            if self._is_length_related_error(error_message):
                print(f"[TTS] 检测到字符长度相关错误: {error_message}")
                print("[TTS] 尝试使用二分法重新处理文本")
    ...
    def _try_bisect_synthesis(self, text: str, voice: Optional[str], language: str, output_path: Optional[str]) -> str:
        """使用二分法尝试合成文本"""
        if len(text) < 2:
        ...
        print(f"[TTS] 二分法处理: 第一段 {len(text1)} 字符, 第二段 {len(text2)} 字符")
        ...
        try:
            ...
            combined.export(output_path, format="wav")
            print(f"[TTS] 二分法合成成功: {output_path}")
