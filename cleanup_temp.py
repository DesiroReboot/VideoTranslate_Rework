...
def cleanup_temp_files(keep_video_path=None):
    ...
    try:
        PathSecurityValidator.validate_path_in_project(str(TEMP_DIR), str(PROJECT_ROOT))
    ...
    if TEMP_DIR.exists():
        ...
        for item in TEMP_DIR.iterdir():
            ...
            try:
                PathSecurityValidator.validate_path_in_project(str(item), str(TEMP_DIR))
            except Exception:
        ...
        print(
            f"temp目录清理完成，删除 {deleted_count} 个，保留 {kept_count} 个文件/目录"
        )
    ...
    if OUTPUT_DIR.exists():
        ...
        for item in OUTPUT_DIR.iterdir():
            ...
            # 保留scoring_results目录
            if item.is_dir() and item.name == "scoring_results":
                print(f"  ⊙ 保留: {item.name}")
                kept_count += 1
                continue
