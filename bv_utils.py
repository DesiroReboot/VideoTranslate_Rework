...
def get_bilibili_video_info_api(bv_id: str, timeout: int = 10) -> Optional[Dict[str, Any]]:
    """
    通过B站API获取视频信息
...
def get_bilibili_video_streams_api(bv_id: str, cid: int, timeout: int = 10) -> Optional[Dict[str, Any]]:
    """
    通过B站API获取视频流信息
...
def get_best_video_stream_from_api(video_stream_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
        ...
        if dash_info:
            ...
            if video_streams:
                ...
                if best_video and audio_streams:
                    ...
                    return {
                        'video_url': best_video.get('baseUrl') or best_video.get('backupUrl', [None])[0],
                        ...
                        'audio_bandwidth': best_audio.get('bandwidth') if best_audio else None,
                        'has_audio': bool(best_audio),
                    }
        
        # 检查是否有flv格式
        accept_quality = video_stream_data.get('accept_quality', [])
        accept_description = video_stream_data.get('accept_description', [])
...
def download_video_directly(video_stream_info: Dict[str, Any], output_path: str, timeout: int = 30) -> bool:
    """
    直接下载视频文件
