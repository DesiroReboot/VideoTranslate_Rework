...
class FileValidator:
    ...
    def validate_video_file(file_path: str) -> Dict[str, Any]:
        ...
        ext = FileValidator.validate_file_extension(
            file_path,
            FileValidator.ALLOWED_VIDEO_EXTENSIONS
        )
        
        # 验证文件大小
        file_size = FileValidator.validate_file_size(
            file_path,
            FileValidator.MAX_VIDEO_SIZE
        )
    ...
    def validate_audio_file(file_path: str) -> Dict[str, Any]:
        ...
        ext = FileValidator.validate_file_extension(
            file_path,
            FileValidator.ALLOWED_AUDIO_EXTENSIONS
        )
        
        # 验证文件大小
        file_size = FileValidator.validate_file_size(
            file_path,
            FileValidator.MAX_AUDIO_SIZE
        )
...
class URLValidator:
    ...
    @staticmethod
    def validate_url_domain(url: str, allowed_domains: Optional[List[str]] = None) -> bool:
        """
        验证URL域名在白名单中（防止SSRF）
...
class RegexValidator:
    ...
    @staticmethod
    def validate_input_length_for_regex(
        input_str: str,
        max_length: int = 500
    ) -> str:
        """
        ...
        if len(input_str) > max_length:
            raise ValueError(f"输入过长，可能导致ReDoS: {len(input_str)} > {max_length}")
