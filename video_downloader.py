...
class BilibiliDebugLogger:
    ...
    def save_raw_response(
        self, url: str, response_text: str, error: Optional[Exception] = None
    ):
        """保存原始响应数据"""
        raw_data = {
    ...
    def log_yt_dlp_debug(self, msg: Dict[str, Any]):
        ...
        if isinstance(msg, dict) and msg.get("url") and msg.get("downloader"):
            self.logger.debug(
                f"yt-dlp下载器: {msg.get('downloader')} - URL: {msg.get('url')}"
            )
...
class VideoDownloader:
    ...
    def download_bilibili_video(
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:  # type: ignore
                ...
                debug_logger.info(
                    f"视频信息提取成功，标题: {info.get('title', '未知')}"
                )
            ...
            try:
                ...
                req = urllib.request.Request(
                    url, headers=ydl_opts.get("http_headers", {})
                )
            ...
            try:
                return VideoDownloader._try_backup_download_strategy(
                    url, bv_id, debug_logger, output_path
                )
            ...
            if "Failed to parse JSON" in str(e):
                debug_logger.error(
                    "检测到'Failed to parse JSON'错误，可能是B站API响应格式问题"
                )
            ...
            try:
                ...
                debug_logger.error("已保存异常信息到原始响应文件")
    ...
    @staticmethod
    def _try_backup_download_strategy(
        url: str,
        bv_id: Optional[str],
        debug_logger: BilibiliDebugLogger,
        ...
        backup_strategies = [
            ...
            },
            {
                ...
                "opts": {
                    ...
                    "geo_bypass": True,
                    "cookiefile": str(BILIBILI_COOKIE_FILE)
        ...
        for i, strategy in enumerate(backup_strategies):
            ...
            try:
                ...
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:  # type: ignore
                    ...
                    debug_logger.info(
                        f"备用策略 {strategy['name']} 成功: {downloaded_file}"
                    )
                    return downloaded_file, bv_id
        ...
        if bv_id:
            try:
                ...
                if (
                    success
                    and os.path.exists(api_output_path)
                    and os.path.getsize(api_output_path) > 0
                ):
                    debug_logger.info(f"B站API直接下载成功: {api_output_path}")
                    return api_output_path, bv_id
                else:
                    debug_logger.warning("B站API直接下载失败或文件无效")
